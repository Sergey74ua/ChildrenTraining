...
class AdvancedSalePostsOnly {
    ...
    public function filter_homepage_posts($query) {
        if ($query->is_home() && $query->is_main_query()) {
            ...
            // Устанавливаем максимальное количество постов только если указано число больше 0
            if (!empty($max_posts) && is_numeric($max_posts) && intval($max_posts) > 0) {
            ...
            function updateFilterValueOptions() {
                ...
                $.post(ajaxurl, data, function(response) {
                    if (response.success) {
                        ...
                        $('#filter_value_description').text(response.data.description);
                    }
                });
            }
    ...
    public function max_count_field_render() {
        $max_posts = get_option('sale_posts_max_count', $this->default_max_posts);
        ?>
        <input type="number" name="sale_posts_max_count" value="<?php echo esc_attr($max_posts); ?>" min="0" max="500" step="1">
        <p class="description">Максимальное количество записей для отображения на главной странице. Укажите 0 чтобы выводить все посты без ограничений.</p>
        <?php
    }
